...
# ========================
#  PALETA DE COLORES GUI
# ========================
BG_MAIN    = "#1f2428"   # fondo ventana
BG_PANEL   = "#272c30"   # paneles / pestañas
BG_HEADER  = "#111417"   # barra superior
FG_TEXT    = "#f5f5f5"   # texto general
FG_MUTED   = "#c0c0c0"   # texto secundario
ACCENT     = "#4CAF50"   # verde principal
ACCENT_2   = "#FF9800"   # acento secundario
DANGER     = "#f44336"   # rojo emergencia
...
class PincherController(Node):
    def __init__(self):
        ...
        # Flag: ¿tenemos hardware Dynamixel disponible?
        # En la máquina virtual normalmente será False (solo simulación).
        ...
        # Si falla, entramos en modo SOLO SIMULACIÓN (no se escribe en los servos).
        self.port = None
        try:
            self.port = PortHandler(port_name)
            if not self.port.openPort():
                self.get_logger().warn(
                    f'No se pudo abrir el puerto {port_name}. '
                    'Se ejecutará SOLO en modo simulación (sin escribir en los servos).'
                )
                self.hardware_available = False
        except Exception as e:
            self.get_logger().warn(
                f'Excepción abriendo el puerto {port_name}: {e}. '
                'Se ejecutará SOLO en modo simulación (sin escribir en los servos).'
            )
            self.hardware_available = False

        # Si se abrió, intentamos configurar baudrate
        if self.hardware_available:
            try:
                if not self.port.setBaudRate(baudrate):
                    self.get_logger().warn(
                        f'No se pudo configurar baudrate={baudrate}. '
                        'Se ejecutará SOLO en modo simululación.'
                    )
                    self.port.closePort()
                    self.hardware_available = False
            except Exception as e:
                self.get_logger().warn(
                    f'Excepción configurando baudrate={baudrate}: {e}. '
                    'Se ejecutará SOLO en modo simulación.'
                )
                self.port.closePort()
                self.hardware_available = False

        # PacketHandler (se puede crear incluso en simulación)
        self.packet = PacketHandler(PROTOCOL_VERSION)

        # Posiciones actuales de las articulaciones (radianes)
        self.current_joint_positions = [0.0] * 5

        # Estado de emergencia
        self.emergency_stop_activated = False

        # Configuración inicial de los motores (si hay hardware)
        self.initialize_motors(goal_positions, moving_speed, torque_limit)

        # --- A PARTIR DE AQUÍ DEJA TU CÓDIGO COMO YA LO TENÍAS ---
        # Publicador para Joint States (para RViz)
        self.joint_state_pub = self.create_publisher(JointState, '/joint_states', 10)
        # ... y todo lo que sigue igual que antes
        ...
        if HAS_POSE_COMMAND and PoseCommand is not None:
            try:
                self.pose_command_pub = self.create_publisher(PoseCommand, 'pose_command', 10)
            except Exception as e:
                # Caso típico: el msg existe pero no tiene typesupport en Python
                self.get_logger().warn(
                    f"No se pudo crear el publisher pose_command (typesupport de PoseCommand ausente): {e}"
                )
        ...
        if self.pose_command_pub is None:
            self.get_logger().warn(
                "pose_command no disponible; la pestaña 'Espacio de la Tarea' solo mostrará un aviso."
            )

    def initialize_motors(self, goal_positions, moving_speed, torque_limit):
        """Configuración inicial de todos los motores."""

        # En modo simulación no tocamos hardware, solo dejamos HOME en RViz.
        if not self.hardware_available:
            self.get_logger().warn(
                "initialize_motors(): Modo solo simulación, no se configuran servos físicos."
            )
        ...
        for dxl_id, goal in zip(self.dxl_ids, goal_positions):
            try:
                ...
                self.get_logger().error(
                    f'Error configurando motor {dxl_id}: {str(e)}'
                )
    ...
    def move_motor(self, motor_id, position):
        """Mueve un motor a la posición especificada.

        Siempre actualiza self.current_joint_positions para que RViz y la GUI
        vean el movimiento incluso en modo simulación.
        """
        ...
        # Actualizar modelo interno SIEMPRE
        try:
            joint_index = self.dxl_ids.index(motor_id)
        except ValueError:
            self.get_logger().error(f'Motor {motor_id} no está en dxl_ids')
            return
        ...
        # Si no hay hardware, solo simulamos
        if not self.hardware_available:
            self.get_logger().debug(
                f"[SIM] Motor {motor_id} -> ticks {position}, "
                f"ángulo {math.degrees(angle):.1f}°"
            )
            return

        # Si hay hardware, escribimos al bus Dynamixel
        try:
    ...
    def update_speed_single_motor(self, motor_id, speed):
        ...
        if not self.hardware_available:
            # En simulación no hay nada que enviar al bus, pero consideramos éxito
            return True
        try:
    ...
    def emergency_stop(self):
        ...
        if not self.hardware_available:
            self.get_logger().warning(
                "Parada de emergencia en modo SIMULACIÓN (sin desactivar servos físicos)."
            )
            return   
    ...
    def reactivate_torque(self):
        ...
        if not self.hardware_available:
            self.get_logger().info(
                "Reactivación de torque en modo SIMULACIÓN (sin hardware)."
            )
            return
        for dxl_id in self.dxl_ids:
            try:
    ...
    def close(self):
        """Apaga el torque (si hay hardware) y cierra el puerto."""
        if self.hardware_available and self.port is not None:
            for dxl_id in self.dxl_ids:
                try:
                    self.packet.write1ByteTxRx(self.port, dxl_id, ADDR_TORQUE_ENABLE, 0)
                except Exception:
...
class PincherGUI:
    def __init__(self, controller):
        ...
        self.window = tk.Tk()
        self.window.title("Interfaz Control Pincher")
        # Fondo global
        self.window.configure(bg=BG_MAIN)

        # Colores por defecto para widgets Tk "clásicos"
        self.window.option_add("*Frame.background", BG_PANEL)
        self.window.option_add("*Label.background", BG_PANEL)
        self.window.option_add("*Label.foreground", FG_TEXT)
        
        # Estilo ttk (tabs diferentes a los de todos)
        style = ttk.Style()
        ...
        # =========================
        #  BARRA SUPERIOR PERSONAL
        # =========================
        header = tk.Frame(self.window, bg=BG_HEADER, height=40)
        header.pack(fill='x', side='top')

        title_label = tk.Label(
            header,
            text="Lab 05 · Cinemática Directa · PhantomX Pincher X100",
            bg=BG_HEADER,
            fg=FG_TEXT,
    ...
    def setup_tab1(self):
        ...
        title_label = tk.Label(
            self.tab1, 
            text="Control por Sliders - TIEMPO REAL", 
        ...
        motors_frame = tk.Frame(self.tab1, bg=BG_PANEL)
        ...
        speed_frame = tk.Frame(self.tab1, bg=BG_PANEL)
    ...
    def setup_tab2(self):
        ...
        for i, motor_id in enumerate(self.controller.dxl_ids):
            ...
            entry = tk.Entry(motor_frame, width=8, font=("Segoe UI", 10))
            ...
            status_label = tk.Label(motor_frame, text="Listo", 
                                   font=("Segoe UI", 9), fg="green", width=10)
    ...
    def setup_tab3(self):
        ...
        title_label = tk.Label(
            self.tab3,
            text="Visualización en RViz - PhantomX Pincher X100",
            font=("Segoe UI", 14, "bold"),
            bg=BG_PANEL,
            fg=FG_TEXT,
        )
        ...
        info_frame = tk.Frame(self.tab3, bg=BG_PANEL)
        ...
        controls_frame = tk.Frame(self.tab3, bg=BG_PANEL)
    ...
    def setup_tab4(self):
        ...
        frame = tk.Frame(self.tab4, bg=BG_PANEL)
        ...
        # Fila 0: Poses 6 y 7 (personalizadas)
        row0 = tk.Frame(grid, bg=BG_PANEL)
        row0.pack(fill='x', pady=5)

        self.pose1_btn = make_button(
            row0,
            "Pose 6 (Personalizada): [10, 20, 30, 40, 50]",
            self.start_pose1_sequence,
        )
        self.pose1_btn.pack(side='left', expand=True, fill='x', padx=5)

        self.pose2_btn = make_button(
            row0,
            "Pose 7 (Personalizada): [-10, -20, -30, -40, -50]",
            self.start_pose2_sequence,
        )
        self.pose2_btn.pack(side='left', expand=True, fill='x', padx=5)

        # Fila 1: Pose Lab 1 y Lab 2
        row1 = tk.Frame(grid, bg=BG_PANEL)
        row1.pack(fill='x', pady=5)

        self.pose_lab1_btn = make_button(
            row1,
            "Pose 1 (Lab): [0, 0, 0, 0, 0]",
            self.start_lab_pose1_sequence,
        )
        self.pose_lab1_btn.pack(side='left', expand=True, fill='x', padx=5)

        self.pose_lab2_btn = make_button(
            row1,
            "Pose 2 (Lab): [25, 25, 20, -20, 0]",
            self.start_lab_pose2_sequence,
        )
        self.pose_lab2_btn.pack(side='left', expand=True, fill='x', padx=5)

        # Fila 2: Pose Lab 3 y Lab 4
        row2 = tk.Frame(grid, bg=BG_PANEL)
        row2.pack(fill='x', pady=5)

        self.pose_lab3_btn = make_button(
            row2,
            "Pose 3 (Lab): [-35, 35, -30, 30, 0]",
            self.start_lab_pose3_sequence,
        )
        self.pose_lab3_btn.pack(side='left', expand=True, fill='x', padx=5)

        self.pose_lab4_btn = make_button(
            row2,
            "Pose 4 (Lab): [85, -20, 55, 25, 0]",
            self.start_lab_pose4_sequence,
        )
        self.pose_lab4_btn.pack(side='left', expand=True, fill='x', padx=5)

        # Fila 3: Pose Lab 5 centrada
        row3 = tk.Frame(grid, bg=BG_PANEL)
        row3.pack(fill='x', pady=5)

        self.pose_lab5_btn = make_button(
            row3,
            "Pose 5 (Lab): [80, -35, 55, -45, 0]",
            self.start_lab_pose5_sequence,
        )
        self.pose_lab5_btn.pack(side='left', expand=True, fill='x', padx=5)

        # Etiqueta de estado de las poses
        ...
        self.pose1_status_label.pack(pady=10)
    ...
    def setup_tcp_status_bar(self):
        ...
        self.tcp_x_label_global = tk.Label(tcp_values_frame, text="X: 0.000", font=("Segoe UI", 9), width=10)
        self.tcp_x_label_global.pack(side='left')

        self.tcp_y_label_global = tk.Label(tcp_values_frame, text="Y: 0.000", font=("Segoe UI", 9), width=10)
        self.tcp_y_label_global.pack(side='left')

        self.tcp_z_label_global = tk.Label(tcp_values_frame, text="Z: 0.000", font=("Segoe UI", 9), width=10)
        self.tcp_z_label_global.pack(side='left')

        self.tcp_r_label_global = tk.Label(tcp_values_frame, text="R: 0.0°", font=("Segoe UI", 9), width=10)
        self.tcp_r_label_global.pack(side='left')

        self.tcp_p_label_global = tk.Label(tcp_values_frame, text="P: 0.0°", font=("Segoe UI", 9), width=10)
        self.tcp_p_label_global.pack(side='left')

        self.tcp_yaw_label_global = tk.Label(tcp_values_frame, text="Y: 0.0°", font=("Segoe UI", 9), width=10)
        ...
        for i in range(1, 6):
            lbl = tk.Label(q_values_frame, text=f"q{i}: 0.0°", font=("Segoe UI", 9), width=9)
            lbl.pack(side='left')
            self.q_labels_global[i] = lbl

    def setup_tab5(self):
        """Configura la pestaña 5: Información del grupo (Acerca de)"""
        info_frame = tk.Frame(self.tab5, bg=BG_PANEL)
        info_frame.pack(fill='both', expand=True, padx=40, pady=40)

        title = tk.Label(
            info_frame,
            text="Laboratorio 5: Cinemática Directa - PhantomX Pincher X100",
            font=("Segoe UI", 14, "bold"),
            bg=BG_PANEL,
            fg=FG_TEXT
        )
        ...
        group_box = tk.Frame(info_frame, bg=BG_MAIN, relief="ridge", bd=2)
        group_box.pack(fill='x', padx=10, pady=10)
    ...
    def setup_tab6(self):
        ...
        def add_field(row, label_text, var, units):
            lbl = tk.Label(fields, text=label_text, font=("Segoe UI", 10), width=10, anchor='e')
            lbl.grid(row=row, column=0, padx=5, pady=3)
            entry = tk.Entry(fields, textvariable=var, width=10, font=("Segoe UI", 10))
            entry.grid(row=row, column=1, padx=5, pady=3)
            u_lbl = tk.Label(fields, text=units, font=("Segoe UI", 9), anchor='w')
    ...
    def launch_rviz(self):
        ...
        try:
            # Lanzar el launch file correcto: phantomx_pincher_description/view.launch.py
    ...
    def update_joints_timer(self):
        ...
        self.window.after(300, self.update_joints_timer)
